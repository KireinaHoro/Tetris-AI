...
public class PlayerThread extends Thread {
  ...
  public PlayerThread(
      String threadName, long randomSeed, double[] weights, AtomicInteger valueToUpdate) {
  ...
  private double computeUtility(
      AdvancedState oldState, AdvancedState newState, int move, int rowsCleared) {
    ...
    double utility =
        weights[Constant.LANDING_HEIGHT] * landingHeight
  ...
  private Utility.IntDoublePair computeUtilityWithLookAhead(
      AdvancedState oldState, AdvancedState newState) {
  ...
  private int pickMove(AdvancedState state, int[][] legalMoves) {
    Utility.IntDoublePair bestUtility =
        new Utility.IntDoublePair(-Integer.MAX_VALUE, -Double.MAX_VALUE);
    int bestMove = 0;
